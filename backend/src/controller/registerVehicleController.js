...
async function createRegisterVehicle(req, res) {
  ...
  const { volumeSize, carBrand, carModel, carLicensePlate, maximumWeight, idCollector } = req.body;
...
const getVehicleCount = async (req, res) => {
  ...
  try {
    const count = await registerVehicleService.getVehicleCount(idCollector);
    res.json(count);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Erro ao buscar quantidade de veículos' });
  }
};
